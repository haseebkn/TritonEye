...
def test_defaults_to_xview3(monkeypatch: Any) -> None:
    monkeypatch.delenv("TRITONEYE_DETECTOR", raising=False)
    assert select_detector({}) == "xview3"
    assert select_detector(cfg()) == "xview3"
...
def test_shipped_config_selects_the_calibrated_sar_backend() -> None:
    # Resolve through the executable selector, not a duplicate implementation.
    import yaml

    root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    conf = yaml.safe_load(open(os.path.join(root, "configs", "model.yaml")))
    assert select_detector(conf) == "xview3"
...
def test_xview3_threshold_env_override(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    config = cfg(xview3_threshold=0.15)
    monkeypatch.setenv("TRITONEYE_XVIEW3_THRESHOLD", "0.05")
    assert detector_threshold(config, "xview3") == 0.05
    monkeypatch.delenv("TRITONEYE_XVIEW3_THRESHOLD")
    assert detector_threshold(config, "xview3") == 0.15
